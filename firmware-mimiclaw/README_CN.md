...
MimiClaw 把一块小小的 ESP32-S3 开发板变成你的私人 AI 助理。插上 USB 供电，连上 WiFi，通过 Telegram、飞书、QQ（经网关桥接）或 WebSocket 跟它对话 — 它能处理你丢给它的任何任务，还会随时间积累本地记忆不断进化 — 全部跑在一颗拇指大小的芯片上。

## 认识 MimiClaw

- **小巧** — 没有 Linux，没有 Node.js，没有臃肿依赖 — 纯 C
- **好用** — 在 Telegram、飞书、QQ 或 WebSocket 发消息，剩下的它来搞定
- **忠诚** — 从记忆中学习，跨重启也不会忘
...
你通过 Telegram、飞书、QQ 或本地 WebSocket 网关发来消息，ESP32-S3 通过 WiFi 收到后送进 Agent 循环 — LLM 思考、调用工具、读取记忆 — 再把回复发回来。同时支持 **Anthropic (Claude)** 和 **OpenAI 兼容接口**，也支持配置自定义 `base_url` 指向非默认端点。一切都跑在一颗 $5 的芯片上，所有数据存在本地 Flash。
...
- 如果要接 QQ：还需要一个 **腾讯官方 QQ 机器人应用**，以及一个对外可访问的 `gateway/` 部署来承接 webhook
...
#define MIMI_SECRET_BASE_URL        ""              // 可选：OpenAI 兼容接口 base URL
#define MIMI_SECRET_SEARCH_KEY      ""              // 可选：Brave Search API key
#define MIMI_SECRET_TAVILY_KEY      ""              // 可选：Tavily API key（优先）
#define MIMI_SECRET_QQ_APP_ID       ""              // 可选：腾讯 QQ 机器人 app id
#define MIMI_SECRET_QQ_APP_SECRET   ""              // 可选：腾讯 QQ 机器人 app secret
#define MIMI_SECRET_QQ_GATEWAY_WS_URL ""            // 可选：例如 "wss://gateway.example.com/qqbot/ws"
#define MIMI_SECRET_PROXY_HOST      "10.0.0.1"      // 可选：代理地址
#define MIMI_SECRET_PROXY_PORT      "7897"           // 可选：代理端口
```

如果 `MIMI_SECRET_BASE_URL` 为空，固件会回退到当前 provider 的默认接口地址。若启用 QQ，`MIMI_SECRET_QQ_GATEWAY_WS_URL` 必须指向云端网关的 `/qqbot/ws` 路由。

本地 onboarding / 管理页 `http://192.168.4.1` 也支持在运行时填写这些字段，包括：
...
- **QQ 桥接** — 通过云端 `gateway/` 的 webhook 和 WebSocket 中继接入腾讯官方 QQ 机器人
...
- **[docs/im-integration/QQBOT_SETUP.md](docs/im-integration/QQBOT_SETUP.md)** — 腾讯官方 QQ 机器人桥接配置说明
