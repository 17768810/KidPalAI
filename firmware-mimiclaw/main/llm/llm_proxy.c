...
#define LLM_ENDPOINT_URL_MAX_LEN 256
...
static bool safe_copy_slice(char *dst, size_t dst_size, const char *src, size_t src_len)
{
    if (!dst || dst_size == 0) {
        return false;
    }
    if (!src || src_len >= dst_size) {
        dst[0] = '\0';
        return false;
    }
...
static const llm_provider_spec_t *llm_provider_spec(void)
{
    static const llm_provider_spec_t openai_spec = {
        .default_url = MIMI_OPENAI_API_URL,
    };
    static const llm_provider_spec_t anthropic_spec = {
        .default_url = MIMI_LLM_API_URL,
    };

    return provider_is_openai() ? &openai_spec : &anthropic_spec;
}

static bool llm_string_has_whitespace(const char *value)
{
    if (!value) {
        return false;
    }
...
static bool llm_string_ends_with(const char *value, const char *suffix)
{
    size_t value_len;
    size_t suffix_len;
...
static bool llm_store_base_url(const char *value, const char *source)
{
    size_t len;
    ...
    len = strlen(value);
    if (len >= sizeof(s_base_url)) {
        ESP_LOGW(TAG,
                 "Ignoring %s base_url: %u bytes exceeds limit %u",
                 source,
                 (unsigned)len,
                 (unsigned)(sizeof(s_base_url) - 1));
        return false;
    }

    safe_copy(s_base_url, sizeof(s_base_url), value);
    return true;
}

static esp_err_t llm_parse_https_url(const char *url, llm_endpoint_t *endpoint)
{
    ...
    if (strlen(url) >= sizeof(endpoint->url)) {
        ESP_LOGE(TAG, "Resolved LLM URL exceeds buffer limit");
        return ESP_ERR_INVALID_SIZE;
    }
    if (llm_string_has_whitespace(url) || strchr(url, '?') || strchr(url, '#')) {
        ESP_LOGE(TAG, "Resolved LLM URL is malformed: %s", url);
        return ESP_ERR_INVALID_ARG;
    }
    if (strncmp(url, https_prefix, strlen(https_prefix)) != 0) {
        ESP_LOGE(TAG, "Resolved LLM URL must use https://: %s", url);
        return ESP_ERR_INVALID_ARG;
    }

    authority = url + strlen(https_prefix);
    path_start = strchr(authority, '/');
    authority_len = path_start ? (size_t)(path_start - authority) : strlen(authority);
    if (authority_len == 0 || memchr(authority, '@', authority_len) != NULL) {
        ESP_LOGE(TAG, "Resolved LLM URL has invalid host: %s", url);
        return ESP_ERR_INVALID_ARG;
    }

    port_sep = memchr(authority, ':', authority_len);
    host_len = port_sep ? (size_t)(port_sep - authority) : authority_len;
    if (host_len == 0 || !safe_copy_slice(endpoint->host, sizeof(endpoint->host), authority, host_len)) {
        ESP_LOGE(TAG, "Resolved LLM host is invalid or too long: %s", url);
        return ESP_ERR_INVALID_SIZE;
    }

    if (port_sep) {
        size_t port_len = authority_len - host_len - 1;
        if (port_len != 3 || strncmp(port_sep + 1, "443", port_len) != 0) {
            ESP_LOGE(TAG, "Resolved LLM URL port is unsupported: %s", url);
            return ESP_ERR_INVALID_ARG;
        }
    }
    ...
    if (path_len >= sizeof(endpoint->path)) {
        ESP_LOGE(TAG, "Resolved LLM path is too long: %s", url);
        return ESP_ERR_INVALID_SIZE;
    }
    safe_copy(endpoint->url, sizeof(endpoint->url), url);
    safe_copy(endpoint->path, sizeof(endpoint->path), path);

    return ESP_OK;
}

static esp_err_t llm_resolve_provider_default_endpoint(llm_endpoint_t *endpoint)
{
    const llm_provider_spec_t *spec = llm_provider_spec();

    if (!spec || !spec->default_url) {
        return ESP_ERR_INVALID_STATE;
    }
    return llm_parse_https_url(spec->default_url, endpoint);
}

static const char *llm_path_suffix_from_default_path(const char *default_path)
{
    ...
    second_slash = strchr(default_path + 1, '/');
    return second_slash ? second_slash : default_path;
}

static esp_err_t llm_build_configured_path(const char *base_path, char *out_path, size_t out_size)
{
    llm_endpoint_t default_endpoint;
    const char *default_path;
    const char *suffix;
    char normalized_base_path[LLM_ENDPOINT_PATH_MAX_LEN];
    const char *effective_base_path = base_path;
    size_t base_len;
    int written;
    esp_err_t err;
    ...
    err = llm_resolve_provider_default_endpoint(&default_endpoint);
    if (err != ESP_OK) {
        return err;
    }
    default_path = default_endpoint.path;
    suffix = llm_path_suffix_from_default_path(default_path);

    if (base_path && base_path[0] != '\0') {
        base_len = strlen(base_path);
        if (base_len >= sizeof(normalized_base_path)) {
            ESP_LOGW(TAG, "Configured base_url path is too long");
            return ESP_ERR_INVALID_SIZE;
        }
        safe_copy(normalized_base_path, sizeof(normalized_base_path), base_path);
        while (base_len > 1 && normalized_base_path[base_len - 1] == '/') {
            normalized_base_path[--base_len] = '\0';
        }
        effective_base_path = normalized_base_path;
    }

    if (!effective_base_path || effective_base_path[0] == '\0' || strcmp(effective_base_path, "/") == 0) {
        safe_copy(out_path, out_size, default_path);
        return ESP_OK;
    }

    if (strcmp(effective_base_path, default_path) == 0 ||
        llm_string_ends_with(effective_base_path, suffix)) {
        safe_copy(out_path, out_size, effective_base_path);
        return ESP_OK;
    }

    base_len = strlen(effective_base_path);
    written = snprintf(out_path,
                       out_size,
                       "%s%s%s",
                       effective_base_path,
                       (base_len > 0 && effective_base_path[base_len - 1] == '/') ? "" : "/",
                       (*suffix == '/') ? suffix + 1 : suffix);
    if (written < 0 || (size_t)written >= out_size) {
        ESP_LOGW(TAG, "Configured base_url path is too long after provider suffix append");
        out_path[0] = '\0';
        return ESP_ERR_INVALID_SIZE;
    }
...
static esp_err_t llm_build_configured_url(char *out_url, size_t out_size)
{
    static const char *https_prefix = "https://";
    const char *authority;
    const char *path_start;
    char resolved_path[LLM_ENDPOINT_PATH_MAX_LEN];
    size_t prefix_len;
    int written;
    esp_err_t err;
    ...
    if (llm_string_has_whitespace(s_base_url) || strchr(s_base_url, '?') || strchr(s_base_url, '#')) {
        ESP_LOGW(TAG, "Configured base_url is malformed: %s", s_base_url);
        return ESP_ERR_INVALID_ARG;
    }
    if (strncmp(s_base_url, https_prefix, strlen(https_prefix)) != 0) {
        ESP_LOGW(TAG, "Configured base_url must start with https://: %s", s_base_url);
        return ESP_ERR_INVALID_ARG;
    }

    authority = s_base_url + strlen(https_prefix);
    path_start = strchr(authority, '/');
    prefix_len = path_start ? (size_t)(path_start - s_base_url) : strlen(s_base_url);
    err = llm_build_configured_path(path_start ? path_start : "", resolved_path, sizeof(resolved_path));
    if (err != ESP_OK) {
        return err;
    }

    written = snprintf(out_url, out_size, "%.*s%s", (int)prefix_len, s_base_url, resolved_path);
    if (written < 0 || (size_t)written >= out_size) {
        ESP_LOGW(TAG, "Configured base_url is too long after normalization");
        out_url[0] = '\0';
        return ESP_ERR_INVALID_SIZE;
    }
...
{
    char configured_url[LLM_ENDPOINT_URL_MAX_LEN] = {0};
    esp_err_t err;
    ...
    if (s_base_url[0] != '\0') {
        err = llm_build_configured_url(configured_url, sizeof(configured_url));
        if (err == ESP_OK) {
            err = llm_parse_https_url(configured_url, endpoint);
            if (err == ESP_OK) {
                endpoint->from_configured_base_url = true;
                ESP_LOGI(TAG,
                         "Resolved LLM endpoint (configured base_url): host=%s path=%s",
                         endpoint->host,
                         endpoint->path);
                return ESP_OK;
            }
        ...
        ESP_LOGW(TAG, "Falling back to provider default endpoint after invalid configured base_url");
    }

    err = llm_resolve_provider_default_endpoint(endpoint);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to resolve LLM endpoint from provider default URL");
        return err;
    }

    endpoint->from_configured_base_url = false;
    ESP_LOGI(TAG,
             "Resolved LLM endpoint (provider default): host=%s path=%s",
             endpoint->host,
             endpoint->path);
    return ESP_OK;
}
...
esp_err_t llm_proxy_init(void)
{
    ...
    if (MIMI_SECRET_BASE_URL[0] != '\0') {
        llm_store_base_url(MIMI_SECRET_BASE_URL, "build default");
    }
    ...
    if (nvs_open(MIMI_NVS_LLM, NVS_READONLY, &nvs) == ESP_OK) {
        ...
        esp_err_t base_url_err = nvs_get_str(nvs, MIMI_NVS_KEY_BASE_URL, base_url_tmp, &len);
        if (base_url_err == ESP_OK && base_url_tmp[0]) {
            llm_store_base_url(base_url_tmp, "NVS");
        } else if (base_url_err == ESP_ERR_NVS_INVALID_LENGTH) {
            ESP_LOGW(TAG,
                     "Ignoring NVS base_url: %u bytes exceeds limit %u",
                     (unsigned)(len > 0 ? len - 1 : len),
                     (unsigned)(sizeof(s_base_url) - 1));
        }
        nvs_close(nvs);
    }
    ...
    if (s_api_key[0]) {
        ESP_LOGI(TAG,
                 "LLM proxy initialized (provider: %s, model: %s, base_url: %s)",
                 s_provider,
                 s_model,
                 s_base_url[0] ? s_base_url : "<provider default>");
    } else {
...
static esp_err_t llm_http_via_proxy(const char *post_data, resp_buf_t *rb, int *out_status)
{
    ...
    proxy_conn_t *conn = proxy_conn_open(endpoint.host, 443, 30000);
    ...
    if (provider_is_openai()) {
        hlen = snprintf(header, sizeof(header),
            ...
            "Content-Length: %d\r\n"
            "Connection: close\r\n\r\n",
            endpoint.path, endpoint.host, s_api_key, body_len);
    } else {
        hlen = snprintf(header, sizeof(header),
            "POST %s HTTP/1.1\r\n"
            "Host: %s\r\n"
            ...
            "Content-Length: %d\r\n"
            "Connection: close\r\n\r\n",
            endpoint.path, endpoint.host, s_api_key, MIMI_LLM_API_VERSION, body_len);
    }
