...
static void dispatch_sentences_to_audio(const char *text)
{
    ...
    while (*p) {
        ...
        for (int i = 0; boundaries[i]; i++) {
            ...
            if (found && (!next_boundary || found < next_boundary)) {
                next_boundary = found;
                boundary_len  = (int)strlen(boundaries[i]);
            }
        }
        ...
        /* Flush if we have a boundary or the buffer is getting full (>=60 bytes) or end of text */
        bool flush = (next_boundary != NULL) || (buf_len >= 60) || (*p == '\0');
        if (flush && buf_len > 0) {
            char *msg = strdup(buf);
            if (msg) {
                if (xQueueSend(g_sentence_queue, &msg, pdMS_TO_TICKS(5000)) != pdTRUE) {
                    ESP_LOGW("agent_loop", "sentence queue full — dropping sentence");
                    free(msg);
                }
            }
            buf_len = 0;
    ...
    char *sentinel = NULL;
    if (xQueueSend(g_sentence_queue, &sentinel, pdMS_TO_TICKS(5000)) != pdTRUE) {
        ESP_LOGW("agent_loop", "failed to send sentinel — audio_task may stale");
    }
}
...
static void agent_loop_task(void *arg)
{
    ...
    while (1) {
        ...
        if (final_text && final_text[0]) {
            ...
            /* Push response to outbound or audio pipeline */
            if (strcmp(msg.channel, MIMI_CHAN_VOICE) == 0) {
                dispatch_sentences_to_audio(final_text);
                free(final_text);
            } else {
                mimi_msg_t out = {0};
