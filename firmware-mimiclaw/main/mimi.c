...
static void outbound_dispatch_task(void *arg)
{
    ...
    while (1) {
        ...
        } else if (strcmp(msg.channel, MIMI_CHAN_QQBOT) == 0) {
            esp_err_t send_err = qqbot_bridge_send_message(msg.chat_id, msg.content);
            if (send_err != ESP_OK) {
...
void app_main(void)
{
    ...
    /* Factory reset: hold GPIO0 low for 3 s at boot → wipe config, start portal */
    if (gpio0_held_long()) {
        config_erase();
        config_server_start();  // noreturn
    }

    /* Apply KidPal LLM credentials (api_key, model) from kidpal NVS into mimi_cfg */
    {
        kidpal_config_t cfg = {0};
        if (config_load(&cfg) == ESP_OK) {
            if (cfg.llm_key[0] || cfg.llm_model[0]) {
                nvs_handle_t h;
                // MimiClaw LLM namespace: "llm_config", keys: "api_key", "model"
                // NOTE: MimiClaw derives the API URL from the "provider" key, not a URL field.
                if (nvs_open(MIMI_NVS_LLM, NVS_READWRITE, &h) == ESP_OK) {
                    if (cfg.llm_key[0])   nvs_set_str(h, MIMI_NVS_KEY_API_KEY, cfg.llm_key);
                    if (cfg.llm_model[0]) nvs_set_str(h, MIMI_NVS_KEY_MODEL,   cfg.llm_model);
                    nvs_commit(h);
                    nvs_close(h);
                }
            }
    ...
    ESP_ERROR_CHECK(qqbot_bridge_init());
...

    {
        ...
        ESP_ERROR_CHECK(qqbot_bridge_start());
        cron_service_start();
        heartbeat_start();
