                                      ...
                                      const char *build_val)
{
    ...
    if (nvs_open(ns, NVS_READONLY, &nvs) == ESP_OK) {
        ...
        if (err == ESP_OK && len > 0) {
            char *buf = malloc(len);
            if (buf) {
                if (nvs_get_str(nvs, nvs_key, buf, &len) == ESP_OK) {
                    value = buf;
                } else {
                    free(buf);
                }
    ...
    const char *effective = value ? value : (build_val ? build_val : "");
    cJSON_AddStringToObject(root, json_key, effective);
    free(value);
}
...
static esp_err_t http_get_config(httpd_req_t *req)
{
    ...
    json_add_effective_config(root, "qq_app_id", MIMI_NVS_QQ, MIMI_NVS_KEY_QQ_APP_ID, MIMI_SECRET_QQ_APP_ID);
    json_add_effective_config(root, "qq_app_secret", MIMI_NVS_QQ, MIMI_NVS_KEY_QQ_APP_SECRET, MIMI_SECRET_QQ_APP_SECRET);
    json_add_effective_config(root, "qq_gateway_ws_url", MIMI_NVS_QQ, MIMI_NVS_KEY_QQ_GATEWAY_WS_URL, MIMI_SECRET_QQ_GATEWAY_WS_URL);
 ...
 */
static esp_err_t nvs_sync_field(cJSON *root, const char *json_key,
                                const char *ns, const char *nvs_key)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGW(TAG, "Failed opening %s: %s", ns, esp_err_to_name(err));
        return err;
    }

    if (item->valuestring[0] == '\0') {
        esp_err_t erase_err = nvs_erase_key(nvs, nvs_key);
        if (erase_err == ESP_OK) {
            ESP_LOGI(TAG, "Cleared %s/%s", ns, nvs_key);
        } else if (erase_err != ESP_ERR_NVS_NOT_FOUND) {
            ESP_LOGW(TAG, "Failed clearing %s/%s: %s", ns, nvs_key, esp_err_to_name(erase_err));
            nvs_close(nvs);
            return erase_err;
        }
    } else {
        esp_err_t set_err = nvs_set_str(nvs, nvs_key, item->valuestring);
        if (set_err != ESP_OK) {
            ESP_LOGW(TAG, "Failed saving %s/%s: %s", ns, nvs_key, esp_err_to_name(set_err));
            nvs_close(nvs);
            return set_err;
        }
    ...
    if (commit_err != ESP_OK) {
        ESP_LOGW(TAG, "Failed committing %s: %s", ns, esp_err_to_name(commit_err));
        nvs_close(nvs);
        return commit_err;
    }
...
static esp_err_t nvs_sync_u16_field(cJSON *root, const char *json_key,
                                    const char *ns, const char *nvs_key)
{
    cJSON *item = cJSON_GetObjectItem(root, json_key);
    if (!item || !cJSON_IsString(item)) return ESP_OK;
    ...
    if (item->valuestring[0] == '\0') {
        ...
        err = nvs_set_u16(nvs, nvs_key, (uint16_t)value);
        if (err != ESP_OK) {
            ESP_LOGW(TAG, "Failed saving %s/%s: %s", ns, nvs_key, esp_err_to_name(err));
            ret = err;
            goto cleanup;
        }
        ESP_LOGI(TAG, "Saved %s/%s", ns, nvs_key);
    ...
    if (touched) {
        err = nvs_commit(nvs);
        if (err != ESP_OK) {
            ESP_LOGW(TAG, "Failed committing %s: %s", ns, esp_err_to_name(err));
            ret = err;
        }
    }
...
static esp_err_t http_post_save(httpd_req_t *req)
{
    ...
    if ((err = nvs_sync_field(root, "ssid",     MIMI_NVS_WIFI,   MIMI_NVS_KEY_SSID)) != ESP_OK) goto save_error;
    if ((err = nvs_sync_field(root, "password", MIMI_NVS_WIFI,   MIMI_NVS_KEY_PASS)) != ESP_OK) goto save_error;

    /* LLM */
    if ((err = nvs_sync_field(root, "api_key",  MIMI_NVS_LLM,    MIMI_NVS_KEY_API_KEY)) != ESP_OK) goto save_error;
    if ((err = nvs_sync_field(root, "model",    MIMI_NVS_LLM,    MIMI_NVS_KEY_MODEL)) != ESP_OK) goto save_error;
    if ((err = nvs_sync_field(root, "provider", MIMI_NVS_LLM,    MIMI_NVS_KEY_PROVIDER)) != ESP_OK) goto save_error;
    if ((err = nvs_sync_field(root, "base_url", MIMI_NVS_LLM,    MIMI_NVS_KEY_BASE_URL)) != ESP_OK) goto save_error;

    /* Telegram */
    if ((err = nvs_sync_field(root, "tg_token", MIMI_NVS_TG,     MIMI_NVS_KEY_TG_TOKEN)) != ESP_OK) goto save_error;

    /* Feishu */
    if ((err = nvs_sync_field(root, "feishu_app_id",     MIMI_NVS_FEISHU, MIMI_NVS_KEY_FEISHU_APP_ID)) != ESP_OK) goto save_error;
    if ((err = nvs_sync_field(root, "feishu_app_secret", MIMI_NVS_FEISHU, MIMI_NVS_KEY_FEISHU_APP_SECRET)) != ESP_OK) goto save_error;

    /* QQ */
    if ((err = nvs_sync_field(root, "qq_app_id",        MIMI_NVS_QQ,     MIMI_NVS_KEY_QQ_APP_ID)) != ESP_OK) goto save_error;
    if ((err = nvs_sync_field(root, "qq_app_secret",    MIMI_NVS_QQ,     MIMI_NVS_KEY_QQ_APP_SECRET)) != ESP_OK) goto save_error;
    if ((err = nvs_sync_field(root, "qq_gateway_ws_url",MIMI_NVS_QQ,     MIMI_NVS_KEY_QQ_GATEWAY_WS_URL)) != ESP_OK) goto save_error;

    /* Proxy */
    if ((err = nvs_sync_field(root, "proxy_host", MIMI_NVS_PROXY, MIMI_NVS_KEY_PROXY_HOST)) != ESP_OK) goto save_error;
    if ((err = nvs_sync_u16_field(root, "proxy_port", MIMI_NVS_PROXY, MIMI_NVS_KEY_PROXY_PORT)) != ESP_OK) goto save_error;
    if ((err = nvs_sync_field(root, "proxy_type", MIMI_NVS_PROXY, MIMI_NVS_KEY_PROXY_TYPE)) != ESP_OK) goto save_error;

    /* Search */
    if ((err = nvs_sync_field(root, "search_key", MIMI_NVS_SEARCH, MIMI_NVS_KEY_API_KEY)) != ESP_OK) goto save_error;
    if ((err = nvs_sync_field(root, "tavily_key", MIMI_NVS_SEARCH, MIMI_NVS_KEY_TAVILY_KEY)) != ESP_OK) goto save_error;
...
save_error:
    ...
    ESP_LOGW(TAG, "Configuration save failed: %s", esp_err_to_name(err));
    return ESP_FAIL;
}
