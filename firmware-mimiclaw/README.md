...
MimiClaw turns a tiny ESP32-S3 board into a personal AI assistant. Plug it into USB power, connect to WiFi, and talk to it through Telegram, Feishu, QQ (via a gateway bridge), or WebSocket — it handles any task you throw at it and evolves over time with local memory — all on a chip the size of a thumb.

## Meet MimiClaw

- **Tiny** — No Linux, no Node.js, no bloat — just pure C
- **Handy** — Message it from Telegram, Feishu, QQ, or WebSocket
...
You send a message through Telegram, Feishu, QQ, or the local WebSocket gateway. The ESP32-S3 picks it up over WiFi, feeds it into an agent loop — the LLM thinks, calls tools, reads memory — and sends the reply back. Supports both **Anthropic (Claude)** and **OpenAI-compatible** providers, including a configurable `base_url` for non-default endpoints. Everything runs on a single $5 chip with all your data stored locally on flash.
...
- Optional for QQ: a **Tencent official QQ Bot app** plus a public `gateway/` deployment for webhook ingress
...
#define MIMI_SECRET_BASE_URL        ""              // optional: OpenAI-compatible endpoint base URL
#define MIMI_SECRET_SEARCH_KEY      ""              // optional: Brave Search API key
#define MIMI_SECRET_TAVILY_KEY      ""              // optional: Tavily API key (preferred)
#define MIMI_SECRET_QQ_APP_ID       ""              // optional: Tencent QQ bot app id
#define MIMI_SECRET_QQ_APP_SECRET   ""              // optional: Tencent QQ bot app secret
#define MIMI_SECRET_QQ_GATEWAY_WS_URL ""            // optional: e.g. "wss://gateway.example.com/qqbot/ws"
#define MIMI_SECRET_PROXY_HOST      ""              // optional: e.g. "10.0.0.1"
#define MIMI_SECRET_PROXY_PORT      ""              // optional: e.g. "7897"
```

If `MIMI_SECRET_BASE_URL` is empty, the firmware falls back to the provider default endpoint. If you configure QQ, `MIMI_SECRET_QQ_GATEWAY_WS_URL` must point at the cloud gateway's `/qqbot/ws` route.

The local onboarding/admin portal at `http://192.168.4.1` exposes the same fields at runtime, including:
...
- **QQ bridge** — Tencent official QQ Bot support through the cloud `gateway/` webhook and WebSocket relay
...
- **[docs/im-integration/QQBOT_SETUP.md](docs/im-integration/QQBOT_SETUP.md)** — Tencent official QQ Bot bridge setup
